...
_THIS_DIR = os.path.abspath(__file__)
_VAULT_DIR = os.path.dirname(_THIS_DIR)
_BACKEND_DIR = os.path.dirname(_VAULT_DIR)
TEMPLATES_DIR = os.path.join(_BACKEND_DIR, "api", "templates")
...
def vault_change_password(
    ...
    NOTE: We do NOT check _is_unlocked() here — the user supplies their
    current_password in the form, which is sufficient proof of identity.
    The beforeunload beacon was firing /vault/lock before this request
    arrived, destroying the session and making _is_unlocked() return False.
    Verifying current_password directly is both correct and more secure.
    """
